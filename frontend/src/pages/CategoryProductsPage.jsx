...
//   const target = allProducts.find(p => p._id === highlightId);
//   if (!target) return;
...
//           className={`px-6 py-2 rounded-full border transition-all duration-200 whitespace-nowrap text-sm font-medium ${activeSubCategory === "all"
...
//             className={`px-6 py-2 rounded-full border transition-all duration-200 whitespace-nowrap text-sm font-medium ${activeSubCategory === sub._id
//               ? "bg-[#F7F3EB] border-[#D16B2D] text-[#D16B2D] shadow-sm"
//               : "bg-white border-gray-300 text-gray-600 hover:border-[#D16B2D]"
//               }`}
//           >
...
//               className={`border shadow-sm transition-all duration-500 ${highlightId === product._id ? "ring-4 ring-[#D16B2D]" : ""
//                 }`}
//             >
//               <img
...
//                   className={`mt-4 w-full border border-black py-2 text-sm transition ${addingProduct === product._id
...
const CategoryProductsPage = () => {
  ...
  //         return categoryId && subCategoryIds.includes(categoryId);
  //       });
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        ...
        // Include parent category ID itself
        const categoryIdsToShow = [slug, ...subCategoryIds];

        const filteredProducts = productRes.data.filter((product) => {
          const categoryId = getCategoryId(product.category);
          return categoryId && categoryIdsToShow.includes(categoryId);
        });
  ...
  const handleSubCategoryClick = (subId) => {
    ...
    if (subId === "all") {
      ...
      const filtered = allProducts.filter(
        (product) => getCategoryId(product.category) === subId
      );
      setProducts(filtered);
    }
  };
  ...
  useEffect(() => {
    if (!highlightId || allProducts.length === 0) return;

    const target = allProducts.find((p) => p._id === highlightId);
    if (!target) return;

    const categoryId = getCategoryId(target.category);
    if (!categoryId) return;

    // auto select sub-category
    setActiveSubCategory(categoryId);
    setProducts(
      allProducts.filter((p) => getCategoryId(p.category) === categoryId)
    );
